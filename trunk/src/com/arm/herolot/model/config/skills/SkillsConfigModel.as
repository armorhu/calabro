package com.arm.herolot.model.config.skills
{
	...
	public class SkillsConfigModel implements IConfigModel
	{
		private var _file:CSVFile;
		
		private var _idCache:Dictionary;
		
		public function SkillsConfigModel()
		{
		...
		private function generateConfigListOfID(id:int):Boolean
		{
			...
			var newConfigs:Vector.<SkillsConfig> = new Vector.<SkillsConfig>();
			...
			while(line < lineNum)
			{
				...
				var newConfig:SkillsConfig = new SkillsConfig();
		 ...
		 * @return
		 */
		public function getSkillsConfigByID(id:int , level:int = 1):SkillsConfig
		{
			var configList:Vector.<SkillsConfig> = getSkillsConfigListByID(id);
		 ...
		 *
		 */
		public function getSkillsConfigByName(name:String , level:int = 1):SkillsConfig
		{
			var configList:Vector.<SkillsConfig> = getSkillsConfigListByName(name);
		...
		public function getSkillsConfigListByID(id:int):Vector.<SkillsConfig>
		{
			if(!(id in _idCache))
			{
		...
		public function getSkillsConfigListByName(name:String):Vector.<SkillsConfig>
		{
			//name to id
			var id:int = _file.nameToID(name);
			return getSkillsConfigListByID(id);
		}

		public function getMaxLevelByID(id:int):int
		{
			var configsList:Vector.<SkillsConfig> = getSkillsConfigListByID(id);
			if(configsList)
			{
