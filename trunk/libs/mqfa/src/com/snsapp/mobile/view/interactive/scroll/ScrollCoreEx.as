package com.snsapp.mobile.view.interactive.scroll
{
	...
	public class ScrollCoreEx extends EventDispatcher
	{
		...
		public function beginDrag():void
		{
			...
			_lastX = _beginX = mouseX;
			_lastY = _beginY = mouseY;
			_speed.x = _speed.y = 0;
...
//			Debugger.log("------Debug-stageMouseUp" + this._target.x);
			...
			{
				_effectiveOffset = getEffectiveOffset(mouseX - _beginX, mouseY - _beginY); //先更新一下有效距离
		...
		{
			this.dispatchEvent(new ScrollHelperEvent(ScrollHelperEvent.EffectiveClick, new Point(mouseX, mouseY)));
		}
...
//			Debugger.log("Debug-endDrag-End" + this._target.x);
		...
		{
			var effectiveOffset:Point = getEffectiveOffset(mouseX - _beginX, mouseY - _beginY);
...
//				Debugger.log("Debug-inertance" + this._target.x);
		...
		protected function rangeTest(flexable:Boolean):Object
		{
			...
			if (_horizontalRange)
			{
				...
				var maxX:Number = _horizontalRange.y; //Math.min(_horizontalRange.y, (_horizontalRange.x + _targetWidth));
				if (flexable)
				{
		...
		{
			return this._targetWidth + _target.x
		}
		...
		{
			return this._targetHeight + _target.y
		}
...
//		public function get horizontalInnner():Boolean{return _horizontalInnner};
		 ...
		 */
		protected function updateScrollParams():void
		{
			_leftOffset = 0;
			...
			_horizontalInnner = this._horizontalRange == null || (_horizontalRange.y - _horizontalRange.x) > this._targetWidth;
			_verticalInner = this._verticalRange == null || (_verticalRange.y - _verticalRange.x) > this._targetHeight;
			if (_horizontalRange)
				_hhh = (_horizontalRange.y - _horizontalRange.x - _targetWidth) * _horizontalSupport;
			if (_verticalRange)
				_vvv = (_verticalRange.y - _verticalRange.x - _targetHeight) * _verticalSupport;
		}

		private static const EVENTLIST:Array = [ //
			ScrollHelperEvent.BeginScroll, //
			ScrollHelperEvent.Scrolling, //
			ScrollHelperEvent.EndScroll, //
			ScrollHelperEvent.ParsueScroll, //
			ScrollHelperEvent.EffectiveClick, //
			ScrollHelperEvent.Swipe];
		...
		protected function get mouseX():Number
		{
			return _stage.mouseX / Starling.contentScaleFactor;
		}

		protected function get mouseY():Number
		{
			return _stage.mouseY / Starling.contentScaleFactor;
		}
	}
}
