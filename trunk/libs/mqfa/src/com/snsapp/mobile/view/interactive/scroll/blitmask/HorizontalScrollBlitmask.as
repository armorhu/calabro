package com.snsapp.mobile.view.interactive.scroll.blitmask
{
	...
	public class HorizontalScrollBlitmask extends ScrollCore
	{
		protected static const MAX_SIZE:uint = 100000;
		...
		/**
		 * @param target
		 * @param range 滚动的可见区域, x属性表示左边界, y表示右边界.
		 * @param flexable
		 */
		public function HorizontalScrollBlitmask(target:DisplayObject, skin:BitmapData, range:Point, flexable:Number = 0)
		{
			_canvas = new Sprite();
			_dirtyBlocks = new Vector.<int>();
			//			_canvas.cacheAsBitmap = true;
			//			_canvas.cacheAsBitmapMatrix = new Matrix();
			_skin = skin;
			
			super(target, range, null, flexable, 0, 1);
		...
		protected function initiatize():void
		{
			...
			_maxblock_count = Math.min(Math.ceil(target.width / blockSize), (Math.ceil(_canvasWidth / blockSize) + 1) * 3);
			if (_canvasWidth > 0 && _canvasHeight > 0)
			{
				...
				//				_canvas.addChild(new Bitmap(_canvasBitmapdata));
			...
			//			var r:DisplayObjectContainer = _parent.root as DisplayObjectContainer;
			//			r.removeChildren();
			//			r.addChild(_canvas);
			//			_canvas.x = 100;
			//			_canvas.y = 200;
			
			//			if (_displayDebugger == null)
			//				_displayDebugger = new DisplayDebugger(_parent.root as DisplayObjectContainer, _canvas);
		}
		...
		private function drawBlock(blockIndex:int):void
		{
			...
			var start:Number = blockIndex * blockSize + _leftOffset;
			matrix.translate(-start, -_topOffset);
			if (bitmapBlocks[blockIndex] == null)
			{
				bitmapBlocks[blockIndex] = new BitmapData(blockSize, _canvasHeight, true, 0);
